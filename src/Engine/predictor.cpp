...
namespace cyclups
{
	...
	void Predictor::Initialise(cvec PredictX, const PairedData &data)
	{
		// for (int i = 0; i < Cons)
		...
		Matrix K = Kernel.GetMatrix(data.X,data.E);
		Store.K = K;
		...
		{
			Vector ki = Kernel.GetVector(PredictX[i],data.X);
		...
		double DKX = Dx.dot(K_decomp.solve(x));
		Store.Prefactor = 1.0/(DKX);
		Store.Delta = K_decomp.solve(Dx);
		Store.Beta = x.dot(Store.Delta);
	}

	
	Prediction Predictor::Predict(cvec predictX, const PairedData &data)
	{
		Initialise(predictX,data); 
		auto B = Constraint.B;
	...
	Prediction Predictor::RegularisedPrediction(cvec predictX, const PairedData &data,  RegularisingFunction Regulariser)
	{
		R = Regulariser;
		...
		Prediction p = Predict(predictX,data);
		Constraint.Remove();
	...
	void Predictor::Optimise(cvec predictX, const PairedData & data)
	{
		...
		if (Optimiser.MaxSteps > 0)
		{
			...
			{
				Vector dLdc = Store.Prefactor * Store.BBt.solve(Constraint.C() - Store.Bp_blups);
				if (UsingRegulariser)
				{
