...
int main(int argc, char**argv)
{
	...
	double signalVariance = 3*0.03*0.03;
	auto K = cyclups::kernel::SquaredExponential(lengthScale,signalVariance);
	auto B = cyclups::basis::Hermite(5);
	auto C = cyclups::constraint::Monotonic();
	//generate sample
	double xmin = -10;
	double xmax = 10;
	int res = 125;
	double dataError = 0.05;
	double skedacity = 10;
	auto D = cyclups::generator::UniformXSample(res,testFunc,xmin,xmax,dataError,skedacity);
	...
	//make predictions
	std::vector<double> tt = JSL::Vector::linspace(xmin,xmax,251);
	auto p = P.Predict(tt,D);
	...
	std::vector<double> testOmega = {0.1};
	for (auto i : testOmega)
	{
		omega = i/pow(tt[1] - tt[0],2);
		auto pp = P.RegularisedPrediction(tt,D,cyclups::RegularisingFunction(R_curve, gradR_curve));

		omega = i/10/pow(tt[1] - tt[0],2);
		auto pp2 = P.RegularisedPrediction(tt,D,cyclups::RegularisingFunction(R, gradR));
